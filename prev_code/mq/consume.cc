...
// 消息回调函数的实现
void Msgcb(AMQP::TcpChannel *channel, const AMQP::Message &message, uint64_t deliveryTag, bool redelivered)
{
    std::string msg;
...
int main()
{
    ...
    // 7.订阅队列消息--设置消息处理回调函数
    auto Callback = std::bind(Msgcb, &channel, std::placeholders::_1, std::placeholders::_2, std::placeholders::_3);
    channel.consume("test-queue", "consume-tag")
        .onReceived(Callback)
        .onError([](const char *message)
                 { std::cout << "订阅test-queue 队列失败！" << message << std::endl; });
