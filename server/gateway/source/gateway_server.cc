...
DEFINE_int32(http_listen_port, 8000, "HTTP服务器监听端口");
DEFINE_int32(websocket_listen_port, 8001, "Websocket服务器监听端口");
...
int main(int argc, char *argv[])
{
    ...
    crin_lc::GatewayServerBuilder gsb;
    gsb.make_redis_object(FLAGS_redis_host,
                          FLAGS_redis_port,
                          FLAGS_redis_db,
                          FLAGS_redis_keep_alive);

    gsb.make_discovery_object(FLAGS_registry_host,
                              FLAGS_base_service,
                              FLAGS_file_service,
    ...
    gsb.make_server_object(FLAGS_websocket_listen_port,
                           FLAGS_http_listen_port);
