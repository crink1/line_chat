...
public:
    ...
    {
        // 新增数据
        etcd::Response resp = _client->put(key, val, _lease_id).get();
        if (resp.is_ok() == false)
        {
...
public:
    ...
    {
        // 先进行服务发现,先获取到当前以及有的数据
        etcd::Response resp = _client->ls(basedir).get();
        if (resp.is_ok() == false)
        {

            LOG_ERROR("获取服务器信息失败: {}", resp.error_message());
        }
