...
DEFINE_string(mysql_db, "crin_lc", "Mysql默认库名称");
...
int main(int argc, char *argv[])
{
    ...
    crin_lc::MessageServerBuilder msb;
    msb.make_mq_object(FLAGS_mq_user,
                       FLAGS_mq_pswd, FLAGS_mq_host,
    ...
    msb.make_mysql_object(FLAGS_mysql_user,
                          FLAGS_mysql_pswd, FLAGS_mysql_host,
                          FLAGS_mysql_db,
                          FLAGS_mysql_cset,
                          FLAGS_mysql_port,
                          FLAGS_mysql_pool_count);

    msb.make_discovery_object(FLAGS_registry_host,
                              FLAGS_base_service,
                              FLAGS_file_service,
                              FLAGS_user_service);

    msb.make_rpc_server(FLAGS_listen_port,
                        FLAGS_rpc_timeout,
                        FLAGS_rpc_threads);
